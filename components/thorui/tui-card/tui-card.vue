<template>
	<view class="tui-card-class tui-card" :class="[full?'tui-card-full':'',border?'tui-card-border':'']" @tap="handleClick"
	 @longtap="longTap">
		<view v-if="headerSlot" class="tui-card-header">
			<slot name="header"></slot>
		</view>
		<view v-else class="tui-card-header" :class="{'tui-header-line':header.line}" :style="{background:header.bgcolor || '#fff'}">
			<view class="tui-header-left" :class="icon.name ? 'flex-center' : ''">
				...
				<text v-if="icon.name" class="tui-icon-class tui-icon tui-header-icon" :class="'tui-icon-' + icon.name" :style="{ color: icon.color || '#999', fontSize: (icon.size || 32) + 'rpx' }"
				></text>
